...
object MainRepository {
    ...
    suspend fun getGirls(page: Int = 1, count: Int = 10) = withContext(Dispatchers.IO) {
        val result = AppNetWorker.netWorker.gankApi.girls(page, count)
        //可以做 缓存数据 等操作
        //...
        result
    }
    ...
    suspend fun getGirls2(page: Int = 1, count: Int = 10) = withContext(Dispatchers.IO) {
        val result = AppNetWorker.netWorker.gankApi.girls(page, count).result()
        //可以做 缓存数据 等操作
        //...
        result
    }
    ...
    suspend fun getBanner() = withContext(Dispatchers.IO) {
        val result = AppNetWorker.netWorker.wanApi.getBanner().result()
        //可以做 缓存数据 等操作
        //...
        result
    }
